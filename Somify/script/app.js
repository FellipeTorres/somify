...
function tocarMusica(base, musica) {
  ...
  tocandoAgoraEl.innerHTML = `
    🎵 Tocando agora: <strong>${musica.title || '(Sem título)'}</strong>
    — ${musica.artist || 'Desconhecido'}
  `;

  // Garante que o player apareça
  exibirPlayer();
}

function exibirPlayer() {
